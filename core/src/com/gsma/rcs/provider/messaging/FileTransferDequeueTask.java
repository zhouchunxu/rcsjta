...
public class FileTransferDequeueTask extends DequeueTask {
    ...
    public void run() {
        ...
        if (logActivated) {
            mLogger.debug("Execute task to dequeue 1-1/1-N and group file transfers");
        }
        String id = null;
        ...
        boolean disp1 = imdnManager.isRequestOneToManyDeliveryDisplayedReportsEnabled();
                    ...
                    switch (state) {
                        case QUEUED:
                            ...
                            if (groupFile) {
                                ...
                                mFileTransferService.dequeueOneToOneFileTransfer(chatId, id, contact,
        ...
        try {
            ...
            while (cursor.moveToNext()) {
                try {
                    ...
                    String id = cursor.getString(fileTransferIdIdx);
                    String contactNumber = cursor.getString(contactIdx);
                    boolean isMulti;
                    contact = contactNumber != null ? ContactUtil
                            .createContactIdFromTrustedData(contactNumber) : null;
                    ...
                    if (groupFile) {
                        ...
                        if (isPossibleToDequeueGroupFileTransfer())
                }else {
                        if (!isPossibleToDequeueOneToOneFileTransfer(contact, file, size)) {
                            setOneToOneFileTransferAsFailedDequeue(contact, id);
                            continue;
